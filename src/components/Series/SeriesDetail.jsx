...
import {
  Tv,Heart,ThumbsUp,ThumbsDown,Clock,Star,Languages,Calendar,ArrowLeft,Film, Layers
} from 'lucide-react';
...
const SeriesDetail = () => {
  ...
  if (!seriesItem) return <div className="text-center py-10">Series not found</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-xl shadow-lg p-6 md:p-10 grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6">
            ...
            <div className="flex items-center text-gray-300">
              {/* <Clock className="w-5 h-5 mr-1" /> */}
                 <Layers className="text-blue-600 w-5 h-5 mr-1" /> {/* Season icon */}
              {seasons} Seasons
            </div>
            <div className="flex items-center text-gray-300">
              {/* <Clock className="w-5 h-5 mr-1" /> */}
              <FaPlay className="text-purple-600 w-5 h-5 mr-1" /> {/* Episode icon */}
              {episodes} Episodes
            </div>
          </div>
       ...
       {/* Trailer */}
{seriesItem.trailerId && (
  <div className="mb-10">
    <h2 className="text-xl font-semibold mb-3">Watch Trailer</h2>
    <div className="aspect-video rounded-lg overflow-hidden">
      <iframe
        src={`https://www.youtube.com/embed/${seriesItem.trailerId}`}
        title={`${title} Trailer`}
        frameBorder="0"
